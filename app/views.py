...
from app.config import create_plot, create_plot1
import pandas as pd
...
# call fonction creat_plot from file config.py  
  plot1 = create_plot(year, departement ,1, 1, "USAGERS ACCIDENTÉS : HOMMES-INDEMNES", 'rgba(230, 36, 36, 0.5)')
  plot2 = create_plot(year, departement,1, 2,"USAGERS ACCIDENTÉS : HOMMES-TUER", 'rgba(4, 0, 247, 0.5)')
  plot3 = create_plot(year, departement, 1,  3,"USAGERS ACCIDENTÉS : HOMMES-BLESSÉES HOSPITALISÉES", 'rgba(54, 214, 71, 0.5)')
  plot4 = create_plot(year, departement,1, 4 ,"USAGERS ACCIDENTÉS : HOMMES-BLESSÉES LEGERÉES",'rgba(0, 0, 0, 0.5)')
  plot5 = create_plot(year, departement ,2, 1, "USAGERS ACCIDENTÉS : FEMMES-INDEMNES", 'rgba(230, 36, 36, 0.5)')
  plot6 = create_plot(year, departement,2, 2,"USAGERS ACCIDENTÉS : FEMMES-TUER", 'rgba(4, 0, 247, 0.5)')
  plot7 = create_plot(year, departement, 2,  3,"USAGERS ACCIDENTÉS : FEMMES-BLESSÉES HOSPITALISÉES  ", 'rgba(54, 214, 71, 0.5)')
  plot8 = create_plot(year, departement,2, 4 ,"USAGERS ACCIDENTÉS : FEMMES-BLESSÉES LEGERÉES",'rgba(0, 0, 0, 0.5)')
  plot9 = create_plot1(year)
# return the tamplate
  return render_template('graph.html', plot1= plot1, plot2 = plot2, plot3 = plot3, plot4 = plot4, plot5 = plot5, plot6 = plot6, plot7 = plot7, plot8 = plot8,plot9=plot9, unhurt = unhurt, dead = dead, 
    hospitalize= hospitalize,hurt_light= hurt_light, men = men, women= women, year=year)
